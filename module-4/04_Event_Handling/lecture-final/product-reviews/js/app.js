...
function displayReviews() {
  if ('content' in document.createElement('template')) { //test browser support for templates
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  //add a listener to the description so if a user clicks on it, w
  // call toggleDescriptionEdit
  const desc = document.querySelector('.description');
  desc.addEventListener('click', (event) => {
    toggleDescriptionEdit(event.target);
  })

  //add a listener to the button to call the hideshowform
  ...
  //add an event listener for when the mouse leaves the input desc
  inputDesc.addEventListener('mouseout', (event) => {
    exitDescriptionEdit(event.target, false);
  })

  //add an event listener to call save review when they click the button
  const btnSave = document.getElementById('btnSaveReview');
  btnSave.addEventListener( 'click', (event) => {
    event.preventDefault();
    //event.stopPropagation(); Stop from continuing to bubble up to parent
    saveReview();
...
function showHideForm() {
  ...
  /*only allowed to click the button once!
  const btnToggle = document.getElementById("btnToggleForm");
  btnToggle.removeEventListener('click', showHideForm);
  */
}
...
function saveReview() {
  ...
  displayReview(review);
   ...
   //push it onto reviews
  //reviews.push(review);
  //display all again
  //displayReviews - but then we'd need to update this to clear out existing
